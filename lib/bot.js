...
module.exports = (function () {
  ...
  bot.prototype = {
    ...
    doEdit(type, title, summary, params, callback) {
      ...
      let doWithToken, doAfterToken;
      doWithToken = function () {
        self.obtainigToken = true;
        self.getToken(title, 'edit', function (err, token) {
          self.obtainigToken = false;
          if (err) {
            callback(err);
            return;
          }
      ...
      doAfterToken = function () {
        ...
        self.api.call(editParams, function (err, data) {
          ...
          if (err === 'Error returned by API: Invalid CSRF token') {
            doWithToken();
          } else if (!err && data.result && data.result === 'Success') {
      ...
      if (!self.editToken) {
        doWithToken();
      } else if (self.obtainigToken) {
        // waint to some else to set token
        let waitingToken = setInterval(() => {
          if (!self.obtainigToken) {
            if (self.editToken) {
